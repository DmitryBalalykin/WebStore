...
namespace WebStore.Clients.Roles
{
    public class RolesClient : BaseClient, IRoleStore<IdentityRole>
    {
        ...
        {
            return await (await PostAsync(_ServiceAddress, role, cancel))
                .Content
        ...
        {
            return await (await PostAsync($"{_ServiceAddress}/Delete", role, cancel))
                .Content
                .ReadAsAsync<bool>(cancel)
                ? IdentityResult.Success
                : IdentityResult.Failed();
        ...
        {
            return await GetAsync<IdentityRole>($"{_ServiceAddress}/FindById/{id}", cancel);
        }

        public async Task<IdentityRole> FindByNameAsync(string name, CancellationToken cancel)
        {
            return await GetAsync<IdentityRole>($"{_ServiceAddress}/FindByName/{name}", cancel);
        }

        public async Task<string> GetNormalizedRoleNameAsync(IdentityRole role, CancellationToken cancel)
        {
            return await (await PostAsync($"{_ServiceAddress}/GetNormalizedRoleName", role, cancel))
                .Content
        ...
        {
            return await (await PostAsync($"{_ServiceAddress}/GetRoleId", role, cancel))
                .Content
                .ReadAsAsync<string>(cancel);
        }
        ...
        {
            return await (await PostAsync($"{_ServiceAddress}/GetRoleName", role, cancel))
                .Content
                .ReadAsAsync<string>(cancel);
        }
        ...
        {
            return await (await PutAsync(_ServiceAddress, role, cancel))
                 .Content
                 .ReadAsAsync<bool>(cancel)
                 ? IdentityResult.Success
                 : IdentityResult.Failed();
